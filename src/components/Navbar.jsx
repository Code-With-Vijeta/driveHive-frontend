...
const Navbar = () => {
 ...
 const changeRole = async () => {
  if (!user) {
    toast.error("You must be logged in to list your car");
    return setShowLogin(true);
  }

  try {
    const { data } = await axios.put('/api/owner/change-role');
  ...
  return (
    ...
    >
      ...
      <button className='sm:hidden cursor-pointer' aria-label="Menu" onClick={()=> setOpen(!open)}>
        <img className="h-7" src={open ? assets.closeIcon:assets.menuIcon} alt="menu" />
      </button>
    </motion.div>
  );
};
